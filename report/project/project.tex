\chapter{An Investigation into the Security of the RobotWeb}
In this chapter, we investigate the behaviour of the RobotWeb under attack. We start by discussing the reasons why an attacker would choose to attack the RobotWeb, using these to predict the aims of an ideal attacker. From then, we derive a mathematical expression to describe the potency of any given attack. Finally, we use this expression to form and test 5 hypotheses about the behaviour of the RobotWeb under attack.

\section{An ideal attacker}
For us to predict the nature of an attacker's behaviour, we must first understand how normal robots behave. A robot always has an estimate of its current position, albeit with some uncertainty, which grows as the robot moves. When the robot moves, it updates its position by combining its past positions with data from its motion sensors, in a process known as odometry.

% Maybe use words instead of numbers?
When one robot $r_1$ encounters another $r_2$, it measures it and creates a factor between its own and $r_2$'s current pose variable. A robot may also encounter a fixed beacon, like a ship encountering a lighthouse. When this happens, the robot will once again create a factor between its and the beacon's current pose variable. After the measurements have been made, each robot will perform several iterations of Gaussian Belief Propagation, to update its position estimates.
...
It is self-evident that all attackers would seek to be effective i.e. their attacks should have a high chance of working especially when no defences are present. In seeking effectiveness in the face of defences, attackers should not want their attacks to be easily identified, as identified attacks can be easily defended against. Thus it stands to reason that attacks are likely to be subtly executed, such that their victims would be able to believe them. For example, an attack that suggests that a small robot has moved 10km in 5s is guaranteed to fail.

Finally, if multiple attackers are present in the environment, it would be best for them to collaborate rather than compete, as it would raise their overall effectiveness. Since competing attackers are less effective than collaborating attackers, we shall spend less time considering them.
...
In the face of this complexity, it becomes clear that simplification is needed.
...
From this, we notice a disturbing detail - the KL divergence is quadratically affected by the $\mb$. This suggests that the attacker's power is unbounded, so long as it chooses an appropriate $\sbsq$. This means that the attacker can convince other robots of any position that it chooses, so long as it displays a high enough confidence.

\subsection{Crafting the perfect message}
From the results derived in the previous section, we know theoretically that an attacker will seek to increase its $\mb$ to $\infty$ and decrease its $\sbsq$ to 0. However, in a real-life scenario, it is unlikely that the attacker would fully exploit these powers, for two reasons.
\begin{enumerate}
...
%todoThis doesn't look right, I think I should be finding $\sbsq$ and $\mb$, or at least some relationship between them. For example as you move $\mb$ further away, you can increase $\sbsq$
...
An intuitive way of defending against attackers is to set an upper bound on how far a message can suggest the robot is from its previous position. 
...
As shown previously, the strength of an attack is highly dependent on its ability to propose arbitrary values of $\Lambda$. This leads to another intuitive defence strategy - robots set an upper bound on the $\Lambda$ values that they receive. We believe that this strategy is effective when there are only a few \textit{identities} spreading misinformation, counterbalanced by many others sending reliable information. In this subsection, we will provide a justification for this belief as well as a strategy that an attacker could take to bypass the upper bound on $\Lambda$.

The $\Lambda$ value of a message can be thought of as its ``pull'', or how strongly the message would move a variable towards its proposed location. The greater the value, the stronger the pull. \todo{Norms?} With this in mind, we can think of the robot's location estimate as being ``pulled'' by good and bad factors, where good factors pull the estimate towards a ground truth, whilst bad factors pull it to some alternate location. 

When sending a message, each robot decides how strongly it wants to pull. Good robots limit their strength to the accuracy of their sensors, while attackers will set their strength to the fullest extent. This once again makes the case for the use of a reasonable upper bound on the strength of a message, similar to the one described above.
...
Considering the ``pull-based model'' of variables, it stands to reason that in a Sybil Attack, the messages sent by individual identities will not be correlated with each other, as that would greatly simplify the detection and prevention of attacks. Instead, we believe that Sybil Attackers would send messages with wildly different $\mu$ values, that when combined, form the attacker's intended attack.
...
We believe that keeping the full position history not only has an adverse impact on the computational performance of a robot but also amplifies the strength of attacks. If an attacker can successfully attack a robot at time $t$, then the pose variable at $t$, $v_t$ will contain a $\mu$ close to the attacker's target $\mu$, and its $\Lambda$ will be high. If $v_t$ is then used to estimate the robot's position at time $t+1$, then it will effectively also attack the robot, as it would further the attacker's belief. The longer the history kept by a robot, the more power an attacker can gain over it.
...
Attackers may not be able to prevent or limit contagion, as they would need to strongly pull unintentional victims back to a ground truth value. Since no robot has absolute certainty about the locations of any other, the attacker will pull them to values close to their ground truth. However, as there is no mechanism to increase $\Lambda$, the attacker will still make the other robots overconfident about their locations, which may bias them in the long term.
...
We will test our hypotheses in a simulated environment, running the RobotWeb. Each robot, including the attacker, will travel along a predefined path. They will all carry a single \textbf{range-bearing} sensor which would measure the distance and bearing between the robot and what it has measured. All attackers will share a common target path, which they will aim to place their victims on.

To link the experimental setup with a real-world application, we will consider the following task. Several patrol robots are patrolling a grid, however, a subset of these robots are attackers, which aim to prevent their victims from discovering a part of the environment. To do this, they will attack all normal robots in the vicinity of the special zone. Normal robots will only communicate with others if they have sensed them, whilst attackers may seek to also communicate with robots that they have not sensed.
