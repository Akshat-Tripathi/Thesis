...
In a spoofing attack, the attacker first finds the ID of a legitimate device, for example by first intercepting packets and then extracting the source ID from them. After this point, the attacker can send packets impersonating the target.

\unsure{Add an example on how it applies to Robot Web?}

\unsure{Should I add an example here?}

\subsubsection{Sybil Attacks}
The Sybil attack allows an attacker to gain undue influence in a peer-to-peer network, by flooding the network with nonexistent peers. Douceur \cite{SybilAttack} models such a network as \dots.
